...
export interface ScanTarget { requestedUrl: string; normalizedUrl: string; origin: string; protocol: 'http' | 'https'; hostname: string; port?: number | null; finalUrl?: string | null; safeDisplayUrl?: string }
export { createScanTarget } from './scan-target.js';
export { LegacyScanTargetError, resolveScanTarget } from './scan-target-resolver.js';
...
export interface CreateScanRequest { url: string; scope?: ScanScope; environmentId?: string; module: QaModule; checks?: string[]; modules?: FullScanModule[]; requestedUrls?: string[]; browsers?: BrowserType[]; viewports?: Viewport[]; options?: ScanOptions }
export interface Scan { id: string; projectId: string; environmentId?: string; createdBy: string; target?: ScanTarget; scope: ScanScope; type: ScanType; module: QaModule; checks: string[]; modules?: FullScanModule[] | undefined; requestedUrls: string[]; viewports: Viewport[]; browsers: BrowserType[]; options: ScanOptions; status: ScanStatus; progress: ScanProgress; createdAt: string; updatedAt: string; startedAt?: string | undefined; completedAt?: string | undefined; cancelledAt?: string | undefined; cancellationRequestedAt?: string | undefined; failureCode?: string | undefined; failureMessage?: string | undefined }
export interface ScanSummary { id: string; projectId: string; environmentId?: string; target?: ScanTarget; scope: ScanScope; module: QaModule; checks: string[]; status: ScanStatus; progress: ScanProgress; createdAt: string }
...
export interface Issue { id: string; projectId: string; detectorId: string; module: QaModule; severity: IssueSeverity; status: IssueStatus; title: string; message: string; fingerprint: string; primaryUrl: string; firstSeenAt: string; lastSeenAt: string; occurrenceCount: number; createdAt: string; updatedAt: string; scanIds?: string[] }
...
export interface PerformanceSnapshot { pageUrl: string; browser: BrowserType; viewport: Viewport; navigation: { ttfbMs: number | null; domContentLoadedMs: number | null; loadMs: number | null }; webVitals: { fcpMs: number | null; lcpMs: number | null; cls: number | null }; network: { requestCount: number; failedRequestCount: number; transferredBytes: number | null; encodedBytes: number | null }; resources: Array<{ url: string; type: string; durationMs: number | null; transferSize: number | null; encodedBodySize: number | null; status: number | null; initiatorType: string | null; renderBlocking: boolean }>; browserVersion?: string }
...
export type EvidenceType = 'SCREENSHOT' | 'VISUAL_ANNOTATION' | 'DOM_SNAPSHOT' | 'NETWORK' | 'CONSOLE' | 'OTHER';
...
export interface VisualElement { ref: string; tagName: string; textPreview: string; role?: string; selector: string; rect: VisualRect; display: string; visibility: string; position: string; overflow: string; overflowX?: string; overflowY?: string; zIndex: string; fontSize: string; lineHeight: string; whiteSpace: string; interactive: boolean; clientWidth?: number; clientHeight?: number; scrollWidth?: number; scrollHeight?: number; lineClamp?: string; textOverflow?: string; paintedAboveRefs?: string[]; }
