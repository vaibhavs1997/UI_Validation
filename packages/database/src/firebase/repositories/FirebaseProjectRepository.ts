...
import { getFirebaseStorage, getFirestoreDb } from '../firebase-admin.js';
...
function project(id: string, data: Record<string, unknown>, environments: Environment[] = []): Project { return { id, name: String(data.name ?? ''), ...(typeof data.baseUrl === 'string' ? { baseUrl: data.baseUrl } : {}), createdBy: String(data.createdBy ?? ''), organizationId: typeof data.organizationId === 'string' ? data.organizationId : null, environments }; }
...
export class FirebaseProjectRepository implements ProjectRepository {
  ...
  async createProject(ownerId: string, input: CreateProjectInput): Promise<Project> {
    ...
    batch.set(projectRef, { id, name: input.name, ...(input.baseUrl ? { baseUrl: input.baseUrl } : {}), createdBy: ownerId, organizationId: null, createdAt: now, updatedAt: now });
    if (input.baseUrl && input.environmentName && input.environmentType) batch.set(environmentRef, { id: environmentId, name: input.environmentName, type: input.environmentType, baseUrl: input.baseUrl, isDefault: true, createdAt: now, updatedAt: now });
  ...
  async deleteProject(ownerId: string, projectId: string): Promise<boolean> {
    ...
    const current = await this.findProjectByIdForUser(ownerId, projectId);
    if (!current) return false;
    const [files] = await getFirebaseStorage().bucket().getFiles({ prefix: `projects/${projectId}/` });
    await Promise.all(files.map((file) => file.delete({ ignoreNotFound: true })));
    await getFirestoreDb().recursiveDelete(projectRef);
    return true;
  }
}
