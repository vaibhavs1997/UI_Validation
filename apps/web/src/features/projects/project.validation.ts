...
export function validateProjectInput(input: CreateProjectRequest): string | null {
  ...
  if (input.baseUrl) { try { const url = new URL(input.baseUrl); if (!['http:', 'https:'].includes(url.protocol)) return 'Use an http or https URL.'; } catch { return 'Enter a valid website URL.'; } }
  if (input.environmentName !== undefined && !input.environmentName.trim()) return 'Enter an environment name.';
  return null;
}
