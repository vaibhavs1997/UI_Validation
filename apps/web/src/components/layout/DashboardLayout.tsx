...
function DashboardFrame({ children, user }: Readonly<{ children: React.ReactNode; user: { name?: string; email: string } }>) {
  ...
  useEffect(() => { const routes = ['/issues', '/scans', '/qa/', '/full-scan', '/schedules', '/reports', '/settings/', '/integrations']; if (!loading && !projects.length && pathname !== '/dashboard' && pathname !== '/projects/new' && routes.some((route) => route.endsWith('/') ? pathname.startsWith(route) : pathname === route)) router.replace('/dashboard'); }, [loading, projects.length, pathname, router]);
  return <div className="dashboard-shell"><button className={`sidebar-backdrop ${sidebarOpen ? 'sidebar-backdrop-visible' : ''}`} aria-label="Close navigation" type="button" onClick={() => setSidebarOpen(false)} /><AppSidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} /><div className="dashboard-content"><header className="dashboard-header"><button className="mobile-menu-button" type="button" aria-label="Open navigation" onClick={() => setSidebarOpen(true)}>Menu</button><div className="dashboard-heading"><span className="dashboard-heading-icon">V</span><div><small>VISIONQA WORKSPACE</small><strong>{selectedProject?.name ?? 'Workspace setup'}</strong></div></div><div className="dashboard-header-actions">{selectedProject && <><HeaderSelect label="PROJECT" value={selectedProject.id} options={projects.map((project) => ({ value: project.id, label: project.name }))} onChange={selectProject} /><HeaderSelect label="ENVIRONMENT" value={selectedEnvironment?.id ?? ''} options={selectedProject.environments.map((environment) => ({ value: environment.id, label: `${environment.name}${environment.isDefault ? ' · Default' : ''}` }))} onChange={selectEnvironment} dot /></>}<button className="dashboard-icon-button" type="button" aria-label="Help" title="Help">?</button><AccountMenu user={user} /></div></header>{error && <div className="dashboard-context-error" role="alert">Unable to load project. <button type="button" onClick={() => window.location.reload()}>Try again</button></div>}<main className="dashboard-main">{children}</main></div></div>;
}
